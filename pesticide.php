...
<ul class="nav nav-tabs  nav-justified">
  ...
  <li data-toggle="tooltip" title="Fertilizer records"><a href="fertilizer.php" style="color:black;">Fertilizer</a></li>
  <li class="active" data-toggle="tooltip" title="Pesticide records"><a href="pesticide.php" style="color:black;">Pesticide</a></li>
</ul>

</nav>

<form method='post' action='pestdel.php'>
<div class="container" style="margin-top:150px">
<div>
<table class="table table-bordered table-striped table-hover" style="width:60%;">
<thead data-toggle="tooltip" title="Pesticide records">
<tr>
        <th> Field ID </th>
...
$query = "SELECT * FROM pesti_used where f_id in(select f_id from field where user='$user') order by date_used DESC";
...
if ($result = $mysqli->query($query)) {
  while ($row = $result->fetch_assoc()) {
      ...
      $field2name = $row["PEST_NAME"];

      $query1 = "SELECT PRICE FROM Pesticide where pest_name = '$field2name'";
      $result1 = $mysqli->query($query1);
      $row1 = $result1->fetch_assoc();
...
<form method="post" action="pestinsert.php">
<div id="myModal" class="modal fade">
  ...
  <div class="form-group">
    <label for="cropname">Pesticide Name</label>
    <select name="field2" class="form-control" data-toggle="tooltip" title="Select Pesticide Name" required>
    <?php

    $query = "SELECT * FROM pesticide";
    if ($result = $mysqli->query($query)) {
    while ($row = $result->fetch_assoc()) {
        $field4name = $row["PEST_NAME"];
    ?>
        <option value='<?= $field4name ?>'><?= $field4name ?></option>
        <?php
        }
...
<?php
if(isset($_SESSION['pestdel'])){
echo'
<div id="snackbar" style="
  background-color: rgb(211,211,211);
  color: red;"><span class="glyphicon glyphicon-warning-sign"></span>'.$_SESSION["pestdel"].'</div>
<script>
...
if(isset($_SESSION['pestinsert'])){
echo'
<div id="snackbar" style="
  background-color: rgb(211,211,211);
  color: green;"><span class="glyphicon glyphicon-ok "></span>'.$_SESSION["pestinsert"].'</div>
<script>
...
if(isset($_SESSION['pestdelete'])){
echo'
<div id="snackbar" style="
  background-color: rgb(211,211,211);
  color: green;"><span class="glyphicon glyphicon-ok "></span>'.$_SESSION["pestdelete"].'</div>
<script>
...
if(isset($_SESSION['pestdelete1'])){
echo'
<div id="snackbar" style="
  background-color: rgb(211,211,211);
  color: red;"><span class="glyphicon glyphicon-warning-sign"></span>'.$_SESSION["pestdelete1"].'</div>
<script>
